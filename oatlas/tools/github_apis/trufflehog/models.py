...
from oatlas.tools.github_apis.trufflehog import log, IGNORE_NOSECRET
from oatlas.utils.common import get_strings, shannon_entropy
...
class Model:
    ...
    def asdict(self):
        ...
        return attr.asdict(self, filter=lambda attr, _: not attr.name.startswith("_"))
...
class File(Model):
    ...
    def read(self) -> str:
        ...
        try:
            ...
            log.warn(f"skipping file '{self.path}': {e}")
            return ""
...
class Entropy(Rule):
    ...
    def findall(self, s: str) -> List[str]:
        ...
        for word in get_strings(s, self._alphabet, self._minlen):
            if shannon_entropy(word, self._alphabet) > self._threshold:
                matched.append(word)
...
class Exclude(Rule):
    ...
    pattern: Optional[re.Pattern] = attr.ib(None, converter=lambda x: re.compile(x) if x else None)
    paths: Optional[List[str]] = attr.ib(None)

    def findall(self, s: str) -> List[str]:
        """Find pattern occurrences in the string."""
        return [m.group() for m in self.pattern.finditer(s)] if self.pattern else []
